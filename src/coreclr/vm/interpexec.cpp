...
void InvokeCompiledMethod(MethodDesc *pMD, int8_t *pArgs, int8_t *pRet)
{
    ...
    CallStubHeader *pHeader = pMD->GetCallStubHeader();
    if (pHeader == NULL)
    {
        pHeader = callStubGenerator.GenerateCallStub(pMD);
        HRESULT hr = pMD->SetCallStubHeader(pHeader);
        if (hr == S_FALSE)
        {
            // We have lost the race for generating the header, so we need to free the one we generated
            // and use the one that was generated by another thread.
            free(pHeader);
            pHeader = pMD->GetCallStubHeader();
        }
    }

    int routineIndex = pHeader->NumRoutines - 1;
    pHeader->Routines[routineIndex] = pMD->GetNativeCode(); // The method to call
