    ...
    if (m_pFreeList)
    {
        ...
        while (pCurrent)
        {
            ...
            if (pCurrent->size >= realSize + reserveForJumpStubs)
            {
                ...
                ExecutableWriterHolderC<TrackAllocation> previousWriterHolder;
                if (pPrevious)
                {
                    previousWriterHolder = ExecutableWriterHolderC<TrackAllocation>::Create(pPrevious, sizeof(TrackAllocation));
                }
